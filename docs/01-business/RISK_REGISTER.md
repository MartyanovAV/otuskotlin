# FitBridge Risk Register

## 1. Реестр рисков
| ID | Риск | Вероятность (1-5) | Влияние (1-5) | Score | Owner | Mitigation | Timeline | Early Warning Indicator | Contingency |
|---|---|---:|---:|---:|---|---|---|---|---|
| R-01 | Низкая конверсия тренеров в оплату | 3 | 5 | 15 | PO/Growth | пересборка onboarding и pricing prompts | 0–3 мес | activation<40% | ручной success-onboarding |
| R-02 | Низкая доля открытия публичных ссылок клиентами | 3 | 4 | 12 | Product/Growth | уточнить messaging тренера, шаблон сообщения и UX публичной страницы | 0–3 мес | link open rate ниже 40% | интервью тренеров и корректировка onboarding-подсказок |
| R-03 | Низкая доля `CompletionMark` после открытия ссылки | 3 | 4 | 12 | Product | упростить отметку выполнения и проверить, не уходит ли обратная связь в мессенджер | 0–3 мес | completion mark rate ниже 35% | добавить qualitative follow-up в пилот без расширения scope |
| R-04 | Недоверие к персональным данным | 4 | 5 | 20 | CEO/Legal | privacy-by-design, consent flows, audit logs | 0–3 мес | вопросы в саппорт по privacy > норма | публичный trust-center и юр. аудит |
| R-05 | Нарушение требований 152-ФЗ | 2 | 5 | 10 | Legal, Architect | legal review, локализация данных, DPIA-подход, шифрование БД, аудит-логи доступа | 0–3 мес и постоянно | замечания юриста/партнёров | временное ограничение чувствительных данных |
| R-06 | Атака конкурентов в нишу независимых тренеров | 3 | 4 | 12 | CEO/Growth | усилить trainer-first positioning и публичный сценарий без клиентской регистрации | 6–12 мес | рост copycat messaging | ускорить партнёрства и бренд |
| R-07 | Перегрузка roadmap | 4 | 3 | 12 | Product | stage-gate на фичи | постоянно | рост WIP и delay | freeze неприоритетных фич |
| R-08 | Слабый adoption ролей в small teams | 2 | 4 | 8 | Sales/Product | пилоты на 2–3 студиях | 6–9 мес | team feature usage < 30% | перенести expansion на позже |
| R-09 | Сложная миграция из текущих инструментов | 4 | 4 | 16 | Product/CS | импорт CSV, шаблоны, migration help | 0–6 мес | drop-off на шаге импорта | concierge migration |
| R-10 | Ошибочное позиционирование как medical product | 2 | 4 | 8 | Legal/Marketing | жёсткие product claims guidelines | 0–3 мес | вопросы партнёров о диагнозах | пересмотр messaging |
| R-11 | Концентрация лидов в 1–2 партнёрах/школах | 3 | 4 | 12 | Growth | cap на долю одного партнёра, параллельный pipeline | 3–9 мес | >35% новых лидов из одного партнёра | перераспределить бюджет в owned channels |
| R-12 | Referral abuse и фиктивные приглашения | 3 | 3 | 9 | Product/Growth | reward только после оплаты или 30 дней активности, anti-fraud review | 0–6 мес | anomalous invite-to-pay ratio | заморозка наград и ручная проверка |
| R-13 | Переоценка willingness-to-pay без встроенного биллинга | 3 | 5 | 15 | Product/Finance | фиксировать только подтверждение оплаты после прохождения критического пути и отдельно помечать интервью как слабый сигнал | 0–3 мес | много позитивных интервью без платёжных обязательств | ручные пилотные договорённости до Phase 2 биллинга |
| R-14 | Scope creep в сторону клиентского кабинета и `AccessGrant` | 4 | 4 | 16 | Product | держать BR-010 каноническим источником MVP; Phase 2-сигналы не добавлять в backlog MVP | постоянно | появление требований регистрации клиента в MVP-задачах | возврат на Gate и пересогласование scope |

## 2. Критические риски
Критическими считаются риски со score 15+:
- R-01,
- R-04,
- R-09,
- R-13,
- R-14.

## 3. Процесс контроля
1. Еженедельный просмотр leading indicators.
2. Ежемесячный risk review на уровне продукта и роста.
3. Ежеквартальный legal/privacy review.

## 4. Риски интерпретации MVP-метрик

1. Link open rate зависит от внешнего канала отправки и привычек тренера; он не является чистой UX-метрикой.
2. Отсутствие `CompletionMark` не всегда означает, что клиент не выполнил план: клиент мог ответить тренеру в мессенджере.
3. Готовность платить из интервью не равна фактической оплате; до Phase 2 биллинга это validation signal.
4. Запросы на клиентскую регистрацию, клиентский кабинет, `AccessGrant` или переносимость истории являются сигналами Phase 2 и не расширяют MVP автоматически.

## 5. Источники
1. КонсультантПлюс, 152-ФЗ — https://www.consultant.ru/document/cons_doc_LAW_61801/
2. BUSINESS_VISION.md.
3. [BR-010-public-plan-link-mvp.md](BR/BR-010-public-plan-link-mvp.md).
