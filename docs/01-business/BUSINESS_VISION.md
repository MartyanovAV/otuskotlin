...
Утверждённый MVP-кандидат — trainer-first MVP с публичной ссылкой на тренировочный план: сначала проверяется узкая B2B-ценность для тренера без регистрации клиента и без полноценной модели прав.
...
MVP с публичной ссылкой на тренировочный план проверяет один базовый рабочий цикл:
1) **Trainer-first сценарий с публичной ссылкой**: тренер регистрируется, создаёт минимальную клиентскую карточку, создаёт тренировочный план, отправляет публичную ссылку, клиент без регистрации открывает план и отмечает выполнение, тренер видит статус и может закрыть доступ по ссылке.
...
Независимый тренер уже ведёт клиентов в мессенджерах и таблицах, но не видит быстрый и проверяемый статус выполнения плана без ручных сообщений. Его проблема в MVP — не отсутствие большой CRM, а отсутствие лёгкого контура «карточка клиента → план → ссылка → отметка выполнения → обзор статуса», который можно запустить без принуждения клиента к регистрации.

### Проверяемые гипотезы MVP

1. **Гипотеза активации тренера:** тренер готов создать `ClientCard` и `TrainingPlan`, если путь до первой публичной ссылки занимает не больше одного рабочего дня.
2. **Гипотеза низкого клиентского friction:** клиент без регистрации чаще открывает план и оставляет `CompletionMark`, чем в сценарии с обязательным аккаунтом.
...
4. **Гипотеза управления доступом:** закрытие публичной ссылки тренером покрывает MVP-потребность контроля доступа без внедрения `AccessGrant`.
...
4. Построить гибридную GTM-модель: trainer-first acquisition и partner-led demand generation → лёгкая клиентская активация по ссылке → будущий client-owned expansion.
...
#### North Star Metric
Количество активных trainer-first связок «клиентская карточка + план + отметка выполнения» с еженедельной активностью.

#### Product metrics
- WAU/MAU тренеров;
- среднее число активных клиентов на платящего тренера;
- time-to-first-plan-link;
- доля активных ссылок с первой отметкой выполнения;
...
- доля планов, по которым тренер получил и фактически отправил публичную ссылку;
- доля открытых публичных ссылок;
- доля открытых ссылок с `CompletionMark`;
- доля тренеров, вернувшихся к просмотру статуса выполнения;
- доля тренеров, закрывших ссылку после завершения доступа или пилота.
...
- median time-to-first-plan-link ≤ 1 день с момента регистрации тренера;
- activation rate тренеров: ≥ 55% достигают «клиентская карточка + план + публичная ссылка» в течение 7 дней;
- доля активных публичных ссылок с первой отметкой выполнения ≥ 50% в течение 14 дней;
...
- Открытие ссылки зависит от внешнего канала отправки и доверия клиента к тренеру, поэтому open rate нельзя трактовать только как качество продукта.
- Отсутствие `CompletionMark` может означать не провал UX, а то, что клиент выполнил план и сообщил тренеру в мессенджере.
