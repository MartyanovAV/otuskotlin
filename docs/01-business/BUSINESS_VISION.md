...
1) **Trainer-first сценарий дневника тренера**: тренер регистрируется, создаёт минимальную клиентскую карточку, создаёт тренировочный план, ищет и повторно использует карточки и планы.
...
Независимый тренер уже ведёт клиентскую базу и планы в мессенджерах и таблицах и тратит время на их поиск и повторное использование. Его проблема в MVP — не отсутствие большой CRM, а отсутствие лёгкого контура «карточка клиента → план → поиск/возврат», который можно запустить без клиентской регистрации и доступа.
...
3. **Гипотеза повторного использования:** поиск и возврат к ранее созданным карточкам и планам достаточно сокращают ручную операционку, чтобы тренер возвращался к продукту.
...
- доля тренеров, архивировавших карточку или план после завершения работы.
...
Trainer-only MVP не отменяет стратегию client-owned данных. Он проверяет более узкий вход: готов ли тренер получать ценность и платить до того, как продукт инвестирует в клиентский аккаунт, публичный или аутентифицированный клиентский доступ, управление правами и переносимую историю. Phase 2 рассматривается только после подтверждения метрик дневника тренера и отдельного решения по scope.
...
| Fit для независимых тренеров и small teams | Ценность trainer-only контура может оказаться недостаточной без будущего клиентского слоя Phase 2 |
...
FitBridge монетизируется через подписку тренеров и малых команд. В trainer-only MVP проверяется только платящий контур тренера; любой клиентский слой не входит в MVP и рассматривается после подтверждения trainer-first ценности.
...
### Приоритетный wedge
Независимые тренеры, ведущие 10–40 клиентов в гибридной или онлайн-модели. Первичное привлечение идёт через тренеров и партнёров; клиент не взаимодействует с системой в текущем MVP.
