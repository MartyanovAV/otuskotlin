...
```mermaid
journey
    title CJM Марии — фитнес-клиента (Trainer-led)
    section Старт (от тренера)
      Получает ссылку от тренера: 4: Мария
      Создаёт профиль: 5: Мария
    section Регулярное использование
      Отмечает выполнение плана: 4: Мария
      Видит динамику: 5: Мария
    section Смена тренера
      Отзывает доступ старому: 4: Мария
      Подключает нового без потери истории: 5: Мария

    title CJM Марии — фитнес-клиента (Product-led)
    section Старт (самостоятельно)
      Хочет вести дневник: 4: Мария
      Скачивает приложение: 5: Мария
    section Заполнение
...
| Этап | Touchpoints | Эмоции | Pain points | Opportunities |
|---|---|---|---|---|
| Осознание (Trainer-led) | ссылка-приглашение от тренера | доверие | не хочет лишних приложений | быстрый вход в готовый план |
| Осознание (Product-led) | сторис, поиск в сторе, друзья | любопытство | сложно начать вести дневник | instant value от первой записи |
