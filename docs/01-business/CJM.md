...
Документ фиксирует пути ключевых персон FitBridge. Канонический объём MVP см. в [MVP_SCOPE_SUMMARY.md](MVP_SCOPE_SUMMARY.md) и [BR-010](BR/BR-010-trainer-diary-mvp.md). Подробно раскрыт выбранный MVP-путь: trainer-only сценарий дневника тренера. Client-owned, Solo-client, client access, multi-specialist и командные сценарии оставлены как Phase 2 / стратегический резерв.
...
| Удержание | Списки и поиск карточек/планов | Уверенность | Вопрос цены и привычки тренера | Показ времени, сэкономленного на поиске и повторном использовании |
...
journey
    ...
    section Удержание
      Возвращается к карточке клиента: 5: Ирина
      Находит и переиспользует план: 4: Ирина
```
...
| Регулярное использование | Списки, карточки клиентов и поиск планов | Контроль | Нужны надёжность и быстрый поиск | Упорядоченный trainer-only рабочий контур |
...
journey
    ...
    section Использование
      Находит планы по клиентам: 5: Алексей
      Повторно использует рабочие данные: 4: Алексей
    section Расширение
...
2. Статусы выполнения и история остаются важной неудовлетворённой потребностью онлайн-коуча и сигналом Phase 2, а не скрытым требованием MVP.
