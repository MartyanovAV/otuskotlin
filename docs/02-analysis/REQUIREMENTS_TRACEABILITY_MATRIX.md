...
Матрица является рабочим источником трассировки для MVP / Gate 1. Отдельные строки со статусом `Needs detail` или `Gap` сохраняют точечные области детализации и не меняют общий статус документа.
...
|---|---|---|---|---|---|---|---|
| RTM-001 | Регистрация клиента и создание клиентского профиля с необязательными `gender` и `goals` | Both | [BR-001](../01-business/BR/BR-001-training-diary.md), `US-R001-001`, `AC-R001-U001-001`, `AC-R001-U001-004`; [BR-009](../01-business/BR/BR-009-consent-privacy-deletion.md), `AC-R009-U001-004` | FR `1.1`, `2.1`; NFR `3.1`, `3.2`, `5.2`, `8.1` | `profile.createClientProfile`, `profile.readClientProfile`, `profile.updateClientProfile` | `TEST-E2E-001`: новый клиент регистрируется и получает пустой клиентский профиль; `TEST-NEG-005`: отсутствие `gender` или `goals` не блокирует onboarding; `heightCm` проверяется как Phase 2/out of MVP | Ready |
...
| RTM-008 | Отметка выполнения тренировки по программе | Both | [BR-004](../01-business/BR/BR-004-program-builder.md), `AC-R004-U001-003`, `AC-R004-U001-004`; [BR-005](../01-business/BR/BR-005-progress-analytics.md), `AC-R005-U002-001` | FR `4.4`, `5.1`, `8.3`; NFR `1.1`, `1.4`, `2.1`, `10.2` | `program.completeWorkout`, `program.trackCompletion`, `diary.createEntry`, `diary.searchEntries` | `TEST-E2E-009`: клиент или тренер с активным доступом отмечает тренировку выполненной, статус виден клиенту и тренеру; профиль клиента тренером не изменяется | Ready |
...
| RTM-016 | Операционное сопровождение пилота без in-product ADMIN/support роли | Both | [BR-002](../01-business/BR/BR-002-access-control.md), `US-R002-003`; [BR-009](../01-business/BR/BR-009-consent-privacy-deletion.md), `US-R009-004`; [MVP Scope](../01-business/MVP_SCOPE_SUMMARY.md) | FR `1.5`, `3.7`; NFR `3.4`, `10.2` | Нет domain API для ADMIN/support в MVP; операции выполняются через controlled operational runbook/provisioning; обычный domain API остаётся CLIENT/TRAINER only | `TEST-OPS-002`: допуск/отзыв пилотного тренера, блокировка пользователя, invite/access edge case и privacy/deletion support action не дают чтение client profile/diary/training history/health-adjacent data и логируются без чувствительных данных | Ready |

## Coverage By MVP Path

| MVP path | Covered traces | Gate 1 acceptance focus |
|---|---|---|
| Trainer-led | `RTM-001`, `RTM-002`, `RTM-003`, `RTM-004`, `RTM-005`, `RTM-007`, `RTM-008`, `RTM-009`, `RTM-010`, `RTM-011`, `RTM-012`, `RTM-013`, `RTM-014`, `RTM-015`, `RTM-016` | Тренер регистрируется, приглашает клиента, клиент подтверждает доступ, тренер читает разрешённые поля профиля, назначает план и ведёт тренировочный процесс, клиент или тренер отмечает выполнение; профиль клиента редактирует только клиент; пилотные support/ops действия не являются продуктовой ролью и не раскрывают чувствительные данные |
| Solo-client | `RTM-001`, `RTM-005`, `RTM-006`, `RTM-008`, `RTM-010`, `RTM-011`, `RTM-012`, `RTM-013`, `RTM-014`, `RTM-016` | Клиент регистрируется самостоятельно, ведёт дневник, создаёт личную программу, назначает её себе и отмечает выполнение; support/ops действия пилота не дают чтение клиентского профиля, дневника или health-adjacent данных |

## Известные разрывы и заметки поддержки

| Gap | Impact | Recommended action |
|---|---|---|
| Матрица классификации данных создана и используется как источник сопоставления классификации и согласий | Базовая трассировка классификации данных, согласий, доступа, логирования и хранения закрыта через `DATA_CLASSIFICATION_MATRIX.md`; открытые вопросы остаются только для детализации Phase 2 | Поддерживать `DATA_CLASSIFICATION_MATRIX.md` вместе с `BR-009`; перед Phase 2 уточнить сроки хранения, гранулярные согласия, хранение медиа, границу платёжных данных, экспорт и удаление данных |
...
| Нет машинно-читаемой привязки к автоматизированным проверкам | Тесты в матрице описаны как целевые сценарии приёмки | При автоматизации приёмки добавить ссылки на test classes/methods |
...
| ADMIN/support scope для MVP ограничен операционным сопровождением пилота | Полноценный in-product ADMIN/support UI, support console и granular operator roles перенесены в Phase 2; MVP operations не дают broad bypass и не раскрывают client profile/diary/training history/health-adjacent data | Поддерживать границу в BR-002, BR-009, MVP scope и FR; архитектурные/API/security документы уточняет Architect отдельно |
