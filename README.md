...
В MVP **не входят**: регистрация клиента, клиентский кабинет, публичные ссылки на план, клиентские отметки выполнения (клиентский дневник), `AccessGrant`, полноценная модель прав, solo-client PLG, multi-specialist сценарий, командный тариф студии, шаблоны программ, биллинг, продвинутая сквозная аналитика и отчётный модуль, замеры, AI-генерация программ, чат, фото/видео, медданные, выделенные API уведомлений (Notification) и аудита (AuditEvent).
...
- обзорный экран (дашборд) со сводными счётчиками базы тренера;
- карточка клиента для тренера;
- создание и завершение простого плана;
...
## DevOps
- [Конфигурация и скрипты развертывания (Docker Compose)](./deploy)
