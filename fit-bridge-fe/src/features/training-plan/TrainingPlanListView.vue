...
const canSubmitEditPlan = computed(
  () =>
    editPlan.value.title.trim().length >= 3 && editPlan.value.items.length > 0 && !isUpdating.value,
)
...
const handleUpdatePlan = async () => {
  ...
  if (!id || !lock || !title) {
    editErrorMessage.value =
      'Данные плана устарели. Пожалуйста, обновите страницу и повторите попытку.'
    return
  }
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-text-main">Тренировочные планы</h1>
    ...
    <div class="flex flex-wrap items-center gap-3 p-3 rounded-xl bg-surface border border-border">
      ...
      <div class="flex-1 min-w-[200px]">
        <Input
          ...
          aria-label="Поиск тренировочных планов по названию"
        ...
        <p class="text-sm text-text-muted mt-1 max-w-sm mx-auto">
          {{
            hasActiveFilters
              ...
              : 'У вас пока нет тренировочных планов. Создайте первый план!'
          }}
        </p>
      </div>
      <Button
    ...
    <!-- Список планов: карточки на телефоне, таблица на desktop -->
    <template v-else>
      <section class="space-y-3 md:hidden" aria-label="Список тренировочных планов">
        <article
          v-for="plan in plans"
          :key="plan.id"
        ...
        >
          <div class="flex items-start gap-3">
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-start justify-between gap-2">
                <h2 class="line-clamp-2 text-base font-semibold text-text-main">
                  {{ plan.title }}
                </h2>
                <PlanStatusBadge :status="plan.status" class="shrink-0" />
              </div>
              <p class="mt-1 text-sm text-text-muted">
                Клиент: {{ getClientNameById(plan.clientCardId) }}
              </p>
          ...
          <div class="mt-4 grid grid-cols-2 gap-2">
            <Button
              ...
              :id="`mobile-view-plan-btn-${plan.id}`"
              >Открыть</Button
            >
            <Button
            ...
            <Button v-else variant="secondary" @click="selectedPlan = plan">Состав</Button>
            <Button
              v-if="plan.status === 'DRAFT'"
              class="bg-success text-text-inverse hover:opacity-90"
              @click="handleActivatePlan(plan)"
              :disabled="isActivating"
            >
              Активировать
            </Button>
            <PlanShareButton :plan="plan" placement="top-left" />
          </div>
        </article>
      </section>
          ...
          <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-border text-text-main">
              <tr
                ...
                @click="selectedPlan = plan"
                @keydown.enter="selectedPlan = plan"
                @keydown.space.prevent="selectedPlan = plan"
                tabindex="0"
                role="button"
                :aria-label="`Открыть тренировочный план ${plan.title ?? 'без названия'}`"
                :id="`plan-row-${plan.id}`"
              >
                <td class="py-4 pl-6 pr-4 font-medium">
                  <div class="flex items-center gap-3">
                    <div
                      class="w-8 h-8 rounded-lg bg-primary-soft text-primary flex items-center justify-center shrink-0"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                ...
                <td class="px-4 py-4">
                  <div class="flex items-center gap-2">
                    <span
                      class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-surface-3 text-[11px] font-medium text-text-muted"
                    >
                ...
                <td class="px-4 py-4 whitespace-nowrap">
                  <div class="flex items-center gap-1.5">
                    <span
                      class="inline-flex items-center justify-center px-2 py-0.5 rounded-md bg-surface-2 text-xs font-semibold text-text-main border border-border/80"
                    >
                      {{ (plan.planItems ?? []).length }} эл.
                    </span>
                ...
                <td
                  ...
                  :title="formatExactCreatedAt(plan.createdAt)"
                >
                ...
                <td class="py-4 pl-4 pr-6 text-right whitespace-nowrap">
                  <div class="flex items-center justify-end gap-2" @click.stop>
                    ...
                    <Button
                      ...
                      variant="default"
                      size="sm"
                      class="h-8 text-xs bg-success text-text-inverse hover:opacity-90"
                    ...
                    <Button
                      v-if="
                        (plan.status === 'ACTIVE' || plan.status === 'DRAFT') && !plan.completedAt
                      "
                      variant="outline"
                      size="sm"
    ...
    <Dialog :open="isCreateOpen" @update:open="isCreateOpen = $event">
      <DialogContent class="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription> Заполните параметры плана и состав тренировки </DialogDescription>
        </DialogHeader>
        ...
        <form @submit.prevent="handleCreatePlan('ACTIVE')" class="space-y-4 py-2">
          <div class="space-y-1.5">
            <label class="text-sm font-medium text-text-main" for="plan-name"
          ...
          <div class="space-y-1.5">
            <label class="text-sm font-medium text-text-main" for="plan-client-select"
              >Клиент *</label
            >
            <select
    ...
    <Dialog :open="isEditOpen" @update:open="isEditOpen = $event">
      <DialogContent class="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
        ...
        <div
          ...
          class="p-3 bg-danger-soft text-danger text-xs rounded-md"
          id="edit-plan-error"
        >
          {{ editErrorMessage }}
        </div>
        ...
        <form @submit.prevent="handleUpdatePlan" class="space-y-4 py-2">
          <div class="space-y-1.5">
            <label class="text-sm font-medium text-text-main" for="edit-plan-name"
              >Название плана *</label
            >
            <Input
              id="edit-plan-name"
          ...
          <div class="space-y-1.5">
            ...
            <div
              class="flex items-center gap-2 p-2.5 rounded-md border border-border bg-surface-3 text-sm text-text-main"
            >
              <span
                class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-surface-2 text-[11px] font-medium text-text-muted"
              >
      ...
      <DialogContent v-if="selectedPlan" class="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
        ...
        <div class="space-y-3 py-2 text-sm">
          ...
          >
            <div class="flex items-center justify-between">
              ...
              <span class="text-xs font-semibold text-text-main">{{
                formatExactCreatedAt(selectedPlan.completedAt)
              }}</span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-xs font-medium text-text-muted">Сложность:</span>
              <span class="text-xs font-semibold text-text-main">{{
                formatDifficulty(selectedPlan.difficulty)
              }}</span>
            </div>
          ...
          <div class="flex justify-between text-xs text-text-faint pb-1">
            ...
            <span class="font-medium text-text-muted">{{
              formatExactCreatedAt(selectedPlan.createdAt)
            }}</span>
          </div>
          ...
          <div class="border-t border-border pt-3 space-y-2.5">
            ...
            <div
              v-if="(selectedPlan.planItems ?? []).length === 0"
        ...
        >
          ...
          <div class="flex items-center justify-end gap-2">
            ...
            <Button
              v-if="
                (selectedPlan.status === 'ACTIVE' || selectedPlan.status === 'DRAFT') &&
