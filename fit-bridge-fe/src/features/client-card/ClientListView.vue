...
import PlanStatusBadge from '@/features/training-plan/ui/PlanStatusBadge.vue'
import {
  type PlanItemDraft,
  mapDraftToPlanItem,
...
const formatDifficulty = (diff?: string) => {
  switch (diff) {
    ...
    case 'NORMAL':
      return 'Нормально'
    case 'HARD':
      return 'Тяжело (с трудом)'
    case 'MAX':
      return 'На пределе возможностей'
    default:
      return 'Не указана'
  }
}
...
const handleUpdateClient = async () => {
  ...
  if (!id || !lock || !displayName) {
    editErrorMessage.value =
      'Данные клиента устарели. Пожалуйста, обновите страницу и повторите попытку.'
    return
  }
...
const handleCreateClientPlan = async (status: TrainingPlanStatus = 'ACTIVE') => {
  ...
  try {
    ...
    planErrorMessage.value =
      err instanceof Error ? err.message : 'Произошла ошибка при создании плана'
  } finally {
...
const handleActivateClientPlan = async (plan: TrainingPlanResponseObject) => {
  ...
  // Подтверждаем необратимое действие: активированный план становится доступен клиенту.
  ...
  try {
    ...
    if (
      ...
      selectedPlanDetails.value.id === plan.id &&
...
<template>
  <div class="space-y-6">
    ...
    <div
      v-else-if="isError"
      class="p-6 rounded-xl border border-danger/30 bg-danger-soft text-center space-y-3"
      role="alert"
    >
    ...
    <div
      v-else-if="clientCards.length === 0"
      class="p-12 text-center border border-dashed border-border rounded-xl bg-surface/50 space-y-4"
    >
      <div
        class="mx-auto w-12 h-12 rounded-full bg-surface-2 flex items-center justify-center text-text-muted"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="24"
        ...
        <p class="text-sm text-text-muted mt-1 max-w-sm mx-auto">
          {{
            searchQuery
              ...
              : 'У вас пока нет клиентов. Создайте первую карточку!'
          }}
        </p>
      </div>
    ...
    <template v-else>
        ...
        >
          <div class="flex items-start gap-3">
            <Avatar class="h-10 w-10 shrink-0 text-xs">
              <AvatarFallback>{{
                (client.displayName ?? 'КЛ').substring(0, 2).toUpperCase()
              }}</AvatarFallback>
            </Avatar>
            <div class="min-w-0 flex-1">
              <div class="flex items-start justify-between gap-2">
                <h2 class="truncate text-base font-semibold text-text-main">
              ...
              <p class="mt-1 line-clamp-2 text-sm text-text-muted">
                {{ client.note || 'Заметок нет' }}
              </p>
              <p
                class="mt-2 text-xs text-text-faint"
                :title="formatExactCreatedAt(client.createdAt)"
              >
                Создан: {{ formatRelativeCreatedAt(client.createdAt) }}
              </p>
            </div>
          </div>
          <div class="mt-4 grid grid-cols-2 gap-2">
            <Button
              ...
              :id="`mobile-open-client-btn-${client.id}`"
            >
              Карточка
            </Button>
            <Button
      ...
      <div
        class="hidden overflow-hidden rounded-xl border border-border bg-surface shadow-xs md:block"
      >
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm">
            <thead
              class="border-b border-border bg-surface-2/60 text-xs font-semibold uppercase tracking-wider text-text-muted"
            >
              <tr>
            ...
            <tbody class="divide-y divide-border text-text-main">
              ...
              >
                ...
                <td class="px-4 py-4 max-w-xs truncate text-xs text-text-muted">
                  <span v-if="client.note" class="truncate block"> 📝 {{ client.note }} </span>
                  <span v-else class="text-text-faint italic"> — </span>
                </td>
                <td
                  class="px-4 py-4 text-xs text-text-muted whitespace-nowrap"
                  :title="formatExactCreatedAt(client.createdAt)"
                >
                  {{ formatRelativeCreatedAt(client.createdAt) }}
                </td>
      ...
      <DialogContent class="sm:max-w-[425px]">
        ...
        <form @submit.prevent="handleCreateClient" class="space-y-4 py-2">
          <div class="space-y-1.5">
            <label class="text-sm font-medium text-text-main" for="client-name"
              >Имя клиента *</label
            >
            <Input
              id="client-name"
          ...
          <div class="space-y-1.5">
            <label class="text-sm font-medium text-text-main" for="client-note"
              >Внутренняя заметка тренера</label
            >
            <Input
              id="client-note"
          ...
          <DialogFooter>
            <Button type="button" variant="outline" @click="isCreateOpen = false"> Отмена </Button>
            <Button
              type="submit"
              :disabled="!newClient.displayName.trim() || isSubmitting"
        ...
        <DialogHeader>
          <div class="flex items-center gap-3 mb-2">
            <Avatar class="h-12 w-12 text-lg">
              <AvatarFallback>{{
                (selectedClient.displayName ?? 'КЛ').substring(0, 2).toUpperCase()
              }}</AvatarFallback>
            </Avatar>
            <div>
        ...
        <div class="space-y-3 py-2 text-sm">
          ...
          <div class="flex justify-between text-xs text-text-faint">
            ...
            <span class="font-medium text-text-muted">{{
              formatExactCreatedAt(selectedClient.createdAt)
            }}</span>
          </div>
          ...
          <div class="border-t border-border pt-4 space-y-3">
            <div class="flex items-center justify-between">
              ...
              <Button
                size="sm"
                variant="secondary"
                @click="openCreatePlanForClient"
                id="add-plan-to-client-btn"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="14"
            ...
            <div v-if="isClientPlansLoading" class="py-6 text-center text-xs text-text-muted">
              <div
                class="inline-block h-5 w-5 animate-spin rounded-full border-2 border-solid border-primary border-r-transparent mr-2 align-middle"
              ></div>
              Загрузка планов...
            </div>
            ...
            <div
              v-else-if="clientPlans.length === 0"
              class="p-5 text-center border border-dashed border-border rounded-lg bg-surface-2/50 space-y-2"
            >
            ...
            <div v-else class="space-y-2 max-h-56 overflow-y-auto pr-1">
              ...
              >
                <div class="space-y-1">
                  <div class="flex items-center gap-2">
                    <span
                      class="font-medium text-sm text-text-main group-hover:text-primary transition-colors"
                    >
                      {{ plan.title }}
                    </span>
                    <PlanStatusBadge :status="plan.status" class="text-[10px] px-1.5 py-0" />
                  </div>
                  <div class="text-xs text-text-muted flex items-center gap-3">
                    ...
                    >
                    <span>•</span>
                    <span :title="formatExactCreatedAt(plan.createdAt)">{{
    ...
    <Dialog :open="isCreatePlanOpen" @update:open="isCreatePlanOpen = $event">
      <DialogContent class="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Назначение плана для клиента:
            <span class="font-semibold text-text-main">{{ selectedClient?.displayName }}</span>
          </DialogDescription>
        </DialogHeader>
        ...
        <form @submit.prevent="handleCreateClientPlan('ACTIVE')" class="space-y-4 py-2">
          <div class="space-y-1.5">
            <label class="text-sm font-medium text-text-main" for="client-plan-name"
              >Название плана *</label
            >
            <Input
              id="client-plan-name"
          ...
          <DialogFooter class="flex flex-col sm:flex-row sm:justify-end gap-2">
            ...
            <Button
              type="button"
              variant="secondary"
              :disabled="!canSubmitClientPlan"
              @click="handleCreateClientPlan('DRAFT')"
              id="save-draft-client-plan-btn"
            >
    ...
    <Dialog
      ...
      @update:open="
        (val) => {
          if (!val) selectedPlanDetails = null
        }
      "
    >
      <DialogContent
        v-if="selectedPlanDetails"
        class="sm:max-w-[550px] max-h-[90vh] overflow-y-auto"
      >
        <DialogHeader>
          <DialogTitle>{{ selectedPlanDetails.title }}</DialogTitle>
          <DialogDescription> Назначен: {{ selectedClient?.displayName }} </DialogDescription>
        </DialogHeader>

        <div class="space-y-3 py-2 text-sm">
          <div class="flex justify-between border-b border-border pb-2">
            <span class="text-text-muted">Статус:</span>
            <PlanStatusBadge :status="selectedPlanDetails.status" />
          </div>

          <!-- Блок с информацией о завершении -->
          <div
            v-if="selectedPlanDetails.completedAt"
            class="p-3 bg-primary-soft/30 rounded-lg border border-primary/20 space-y-1 mb-2"
          >
            <div class="flex items-center justify-between">
              <span class="text-xs font-medium text-text-muted">Завершена:</span>
              <span class="text-xs font-semibold text-text-main">{{
                formatExactCreatedAt(selectedPlanDetails.completedAt)
              }}</span>
            </div>
            <div v-if="selectedPlanDetails.difficulty" class="flex items-center justify-between">
              ...
              <span class="text-xs font-semibold text-text-main">{{
                formatDifficulty(selectedPlanDetails.difficulty)
              }}</span>
            </div>
            <div
              v-if="selectedPlanDetails.coachComment"
              class="pt-2 mt-2 border-t border-primary/10"
            >
              <span class="text-xs font-medium text-text-muted block mb-1"
                >Комментарий тренера:</span
              >
              <p class="text-xs text-text-main italic whitespace-pre-wrap">
                {{ selectedPlanDetails.coachComment }}
              </p>
            </div>
          </div>
          ...
          <div class="flex justify-between text-xs text-text-faint pb-1">
            <span>Дата создания:</span>
            <span class="font-medium text-text-muted">{{
              formatExactCreatedAt(selectedPlanDetails.createdAt)
            }}</span>
          </div>
          ...
          <div class="border-t border-border pt-3 space-y-2.5">
            ...
            <div
              v-if="(selectedPlanDetails.planItems ?? []).length === 0"
        ...
        <DialogFooter
          class="flex flex-col sm:flex-row sm:justify-between items-stretch sm:items-center w-full gap-2 pt-2"
        >
          <div>
    ...
    <Dialog :open="isEditOpen" @update:open="isEditOpen = $event">
      <DialogContent class="sm:max-w-[425px]">
        ...
        <form @submit.prevent="handleUpdateClient" class="space-y-4 py-2">
          <div class="space-y-1.5">
            <label class="text-sm font-medium text-text-main" for="edit-client-name"
              >Имя клиента *</label
            >
            <Input
              id="edit-client-name"
          ...
          <div class="space-y-1.5">
            <label class="text-sm font-medium text-text-main" for="edit-client-note"
              >Внутренняя заметка тренера</label
            >
            <Input
              id="edit-client-note"
          ...
          <DialogFooter>
            <Button type="button" variant="outline" @click="isEditOpen = false"> Отмена </Button>
