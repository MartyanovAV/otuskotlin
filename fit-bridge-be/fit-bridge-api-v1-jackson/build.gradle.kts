...
dependencies {
    ...
    specsFromLib("${libs.fit.bridge.specs.get()}:spec@zip")
...
openApiGenerate {
    ...
    // 4. Указываем конкретный файл внутри распакованной папки (specs-fitbridge-v1.yaml)
    inputSpec.set(specDir.map { it.file("specs-fitbridge-v1.yaml").asFile.absolutePath })
