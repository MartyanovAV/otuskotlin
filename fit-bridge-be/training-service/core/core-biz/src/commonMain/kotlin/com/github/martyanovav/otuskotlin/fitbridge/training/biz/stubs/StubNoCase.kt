...
fun <C : IFBContext> ICorChainDsl<C>.stubNoCase(title: String) =
    worker {
        this.title = title
        this.description =
            """
        ...
        handle {
            ...
            addError(
                FBError(
                    code = "stub-not-configured",
                    group = "business",
                    field = "",
                    message = "Select a stub case in debug settings or unsupported case: ${stubCase.name}",
                ),
            )
        }
    }
