...
fun ClientCardContext.toTransport(): Any =
    when (command) {
...
private fun ClientCardSearchFilter?.toInternal() =
    ClientCardFilter(
...
private fun ClientCardStatusV2?.toClientCardStatus() =
    when (this) {
...
fun ClientCardContext.toTransportClientCardCreate() =
    ClientCardCreateResponse(
...
fun ClientCardContext.toTransportClientCardRead() =
    ClientCardReadResponse(
        requestId = requestId.takeIf { it != RequestId.NONE }?.asString(),
        result = if (state == State.RUNNING || state == State.FINISHING) ResponseResult.SUCCESS else ResponseResult.ERROR,
        errors = errors.toTransportErrors(),
        clientCard = clientCardResponse.toTransportClientCard()
...
fun ClientCardContext.toTransportClientCardUpdate() =
    ClientCardUpdateResponse(
        requestId = requestId.takeIf { it != RequestId.NONE }?.asString(),
        result = if (state == State.RUNNING || state == State.FINISHING) ResponseResult.SUCCESS else ResponseResult.ERROR,
        errors = errors.toTransportErrors(),
        clientCard = clientCardResponse.toTransportClientCard()
...
fun ClientCardContext.toTransportClientCardArchive() =
    ClientCardArchiveResponse(
        requestId = requestId.takeIf { it != RequestId.NONE }?.asString(),
        result = if (state == State.RUNNING || state == State.FINISHING) ResponseResult.SUCCESS else ResponseResult.ERROR,
        errors = errors.toTransportErrors(),
        clientCard = clientCardResponse.toTransportClientCard()
...
fun ClientCardContext.toTransportClientCardSearch() =
    ClientCardSearchResponse(
        requestId = requestId.takeIf { it != RequestId.NONE }?.asString(),
        result = if (state == State.RUNNING || state == State.FINISHING) ResponseResult.SUCCESS else ResponseResult.ERROR,
        errors = errors.toTransportErrors(),
...
fun ClientCard.toTransportClientCard(): ClientCardResponseObject? {
...
private fun ClientCardCreateObject?.toInternal() =
    ClientCard(
        displayName = this?.displayName.orEmpty(),
        note = this?.note.orEmpty()
    )

private fun ClientCardReadObject?.toInternal() =
    ClientCard(
        id = this?.id.toClientCardId()
    )

private fun ClientCardUpdateObject?.toInternal() =
    ClientCard(
        id = this?.id.toClientCardId(),
        displayName = this?.displayName.orEmpty(),
        note = this?.note.orEmpty(),
