...
class ClientCardValidationRuleUnitTest {
    ...
    @Test
    fun `display name must not be empty`() =
        runTest {
            assertRule(
    ...
    @Test
    fun `display name must satisfy maximum length`() =
        runTest {
            assertRule(
    ...
    @Test
    fun `display name must contain letters`() =
        runTest {
            assertRule(
    ...
    @Test
    fun `note must satisfy maximum length`() =
        runTest {
            assertRule(
    ...
    private suspend fun assertRule(
        ...
        expectedCode: String,
        rule: ICorChainDsl<ClientCardContext>.() -> Unit,
    ) {
        val context =
            ClientCardContext(
                state = State.RUNNING,
